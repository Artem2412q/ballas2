(() => {
  ...
  // Default endpoint can be configured once in rp/sheet/config.js
  ...
  // --------- Steps ---------
  const STEPS = [
    { id: 'profile', href: 'index.html', fields: ['name', 'alias', 'dob', 'role', 'set', 'tags'] },
    { id: 'backstory', href: 'backstory.html', fields: ['backstory'] },
    { id: 'traits', href: 'traits.html', fields: ['traits', 'limits'] },
    { id: 'connections', href: 'connections.html', fields: ['connections'] },
    { id: 'goals', href: 'goals.html', fields: ['goals', 'secret'] },
    { id: 'scenes', href: 'scenes.html', fields: ['wishes'] },
  ];
  ...
  const initScenesPage = () => {
    ...
    submitBtn?.addEventListener('click', async () => {
      ...
      if (!filled(endpoint)) {
        setStatus(submitStatus, 'Сцены готовы, но отправка в Google Sheets не настроена (endpoint пустой). Админ: добавь URL в rp/sheet/config.js или открой ?setup=1.', 'warn');
        return;
      }
